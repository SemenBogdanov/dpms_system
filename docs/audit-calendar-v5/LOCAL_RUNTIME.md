...
Состояние на 14.09.2026: кандидатные frontend/backend запущены на loopback-портах,
схема093, локальные данные сохранены без повторного seed. Три прежних worker остаются остановлены.
Ветка `feature/calendar-availability-locks`: закрытие дней, заявки, сводка доступности,
новая встреча на 30 минут и архивирование только системным admin.
Дополнение: независимый доступ, ячейковые предусловия конкурентной записи,
нормы групп, отбор групп по времени, персональные замечания и отчет загрузки
относительно свободного времени. Полный отчет: `PLANNING_FOLLOWUP_VERIFICATION.md`.
Проверки дополнения: `AVAILABILITY_CONTROL_VERIFICATION.md`.
Подбор доступных окон: `MEETING_WINDOWS_VERIFICATION.md`.
Предыдущий отчет V5 `VERIFICATION.md` сохраняет результаты исходного выпуска.
