...
- Общий deadline локальной генерации 900 секунд. Каноническая атомизация DPMS
  ожидает 930 секунд, а worker lease составляет 1200 секунд и обновляется перед
  каждой попыткой. Обычные облачные вызовы сохраняют отдельный timeout 90 секунд.
...
Оператор останавливает шлюз и запускает `launch_agent.py recover`. Команда дважды
проверяет `/slots`, требует полностью свободную llama и архивирует конкретный
несекретный marker в приватном state-каталоге без чтения его содержимого. Затем
шлюз можно запустить снова. Одного перезапуска недостаточно.

```sh
cd /Users/bogdanov/Code/dpms-id64a/deploy/local-llm
.venv/bin/python launch_agent.py stop
.venv/bin/python launch_agent.py status
.venv/bin/python launch_agent.py recover
.venv/bin/python launch_agent.py start
.venv/bin/python launch_agent.py status
```

`recover` отказывается работать, пока gateway слушает порт, если модель занята,
если `/slots` не подтвержден или marker имеет неожиданный тип, владельца, права
или размер.
