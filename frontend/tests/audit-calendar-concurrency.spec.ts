...
const participant = (page: Page) => region(page).getByRole('combobox', { name: 'Участник', exact: true })
...
const saved = (page: Page) => expect(region(page).getByRole('status')).toContainText('Сохранено интервалов:')
...
test('whole-day clear sends exact 48-cell baseline and preserves mixed values', async ({ page }) => {
  const state = fixtureState()
  state.availability.push({ user_id: ids.a, date: day, start: 600, end: 660, available: true }, { user_id: ids.a, date: day, start: 660, end: 690, available: false })
  ...
  expect(payload.expected?.slice(20, 24).map(cell => cell.value)).toEqual([true, true, false, null])
...
test('uncertain committed write replays UUID and byte-identical body after unrelated refresh and day lock', async ({ page }) => {
  ...
  await slot(page).click()
  await expect(region(page).getByRole('status')).toContainText('Сохранение не подтверждено')
...
test('single in-flight protects actor, dates and cells through the success refresh', async ({ page }) => {
  ...
  try {
    await slot(page).evaluate((button: HTMLButtonElement) => { button.click(); button.click() })
    ...
    await expect(page).toHaveURL(url)
    releaseWrite()
    await expect(region(page).getByRole('status')).toContainText('Обновление данных')
    await expect(slot(page, day, 630)).toBeDisabled()
    expect(commands).toHaveLength(1)
    releaseRead()
    await saved(page)
