...
import { CalendarDays, Clock3, Database, Users, Settings2, History, Upload, ChevronLeft, ChevronRight, RefreshCw, Search, HelpCircle } from 'lucide-react'
import { auditCalendar, type CalendarFact, type CalendarPlan, type CalendarState } from '@/api/auditCalendar'
import { ApiError } from '@/api/client'
import { addDays, calendarTargetScopes, clampDay, errorText, MAX_DATE, MIN_DATE, moscowToday, numberLabel, periodError } from '@/lib/auditCalendar'
...
type MeetingSelection = { date: string; start: number; plan?: CalendarPlan; fact?: CalendarFact }
export function AuditCalendarPage() {
  ...
  const to = urlError ? clampDay(addDays(initialToday, 6)) : rawTo
  ...
  const refresh = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const result = await auditCalendar.state({ from, to: queryTo, ...(group && { group }), ...(person && view !== 'availability' && { person }), ...(q && { q }) })
      ...
      if (sequence === requestSequence.current) {
        ...
        if (forbidden) { setMeeting(null); setAction(null); setRestore(null); setData(null) }
      }
      throw e
  ...
  useEffect(() => {
    const revoke = () => { setData(null); setMeeting(null); setAction(null); setRestore(null); setDenied(true); setError('Доступ отозван. Обратитесь к администратору контура.') }
  ...
  const applyPeriod = (first: string, last: string) => {
    ...
    setValidation(''); update({ from: first, to: last, day: first })
  }
  ...
  return <div className="ac ac-page"><header className="ac-page-head"><div><h1>Сетевой план-график</h1><p className="ac-muted">{data?.scope.name || 'Календарь аудита'} · Europe/Moscow</p></div>{data && !denied && <div className="ac-kpis" aria-label="Показатели периода">{([{ key: 'plan', label: 'План' }, { key: 'fact', label: 'Факт' }, { key: 'attention', label: 'Внимание' }, { key: 'target', label: 'Цель' }] as const).map(k => <div key={k.key} className={`ac-kpi ac-kpi-${k.key}`}><strong>{numberLabel(data.stats[k.key])}</strong><span>{k.label}</span></div>)}</div>}</header>
    <div className="ac-workspace"><aside className="ac-local-column"><div className="ac-local-context"><CalendarDays size={22} /><strong>Аудит</strong><span>{data?.scope.archived ? 'Архив' : data?.actor.can_manage ? 'Помощник' : 'Просмотр'}</span></div><nav aria-label="Представления календаря">{views.filter(v => !v.helper || data?.actor.can_manage).map(({ id, label, Icon }) => { const next = new URLSearchParams(params); next.set('view', id); return <Link key={id} to={`?${next}`} aria-current={view === id ? 'page' : undefined}><Icon size={18} /><span>{label}</span></Link> })}</nav></aside>
      <main className="ac-content" aria-busy={loading}>
        ...
        {data && !denied && <div className="ac-toolbar"><label>Группа<select value={group} onChange={e => update({ group: e.target.value })}><option value="">Все группы</option>{data.groups.map(g => <option key={g.id} value={g.id}>{g.code}</option>)}</select></label>{view !== 'availability' && <label>Участник<select value={person} onChange={e => update({ person: e.target.value })}><option value="">Все участники</option>{data.members.map(m => <option key={m.user_id} value={m.user_id}>{m.code} · {m.full_name}</option>)}</select></label>}<form className="ac-search" onSubmit={e => { e.preventDefault(); update({ q: search.trim() }) }}><label><span className="sr-only">Поиск активности</span><input type="search" value={search} maxLength={200} placeholder="Активность…" onChange={e => setSearch(e.target.value)} /></label><button className="ac-icon" type="submit" title="Найти" aria-label="Найти"><Search size={16} /></button></form><div className="ac-legend"><span className="ac-plan-label">План</span><span className="ac-fact-label">Факт</span><span>△ Внимание</span></div></div>}
        {error && <div className="ac-error" role="alert"><strong>{denied ? 'Нет доступа к контуру' : 'Данные не обновлены'}</strong><p>{error}</p><button type="button" onClick={() => void refresh().catch(() => undefined)}>Повторить</button></div>}
        {loading && !data && <p className="ac-empty" role="status">Загрузка календаря…</p>}
        {data && !denied && <div className="ac-bound-content" ref={node => node?.toggleAttribute('inert', loading || dataQuery !== query)} aria-busy={loading || dataQuery !== query}><p className="ac-scope-caption">Цель: {calendarTargetScopes[data.stats.target_scope]}. Поиск и участник не меняют цель и накопленный недобор.{data.scope.archived ? ' Контур архивирован, изменения заблокированы.' : ''}</p>
          {view === 'graph' && <CalendarGraph state={data} from={from} to={to} day={selectedDay} groupId={group || null} setDay={day => update({ day })} onOpen={open} />}
          {view === 'availability' && <><CalendarAvailability state={data} from={from} person={params.get('availability_person') || data.actor.user_id} setPerson={availability_person => update({ availability_person })} day={selectedDay} setDay={day => update({ day })} onRefresh={refresh} onAbsence={user => setAction({ kind: 'absence', user })} /><h3>Отсутствия</h3><AbsenceList state={data} onAction={setAction} /></>}
          ...
          {view === 'management' && data.actor.can_manage && <CalendarManagement state={data} onAction={setAction} />}
          ...
          {views.find(v => v.id === view)?.helper && !data.actor.can_manage && <p className="ac-empty">Представление доступно помощнику контура.</p>}
          {view === 'help' && <CalendarHelp />}
        </div>}
      </main>
    </div>
    {data && !denied && meeting && <CalendarMeetingEditor state={data} {...meeting} onClose={() => setMeeting(null)} onRefresh={refresh} />}
